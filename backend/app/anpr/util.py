...
def read_license_plate(license_plate_crop):
    ...
    try:
        ...
        if h < 80:
            ...
            scale = 200 / h
            processed_crop = cv2.resize(license_plate_crop, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)
        else:
        ...
        if settings.OCR_DEBUG_SAVE_CROPS:
            os.makedirs(settings.OUTPUT_DIR, exist_ok=True)
            cv2.imwrite(os.path.join(settings.OUTPUT_DIR, "debug_last_crop.jpg"), processed_crop)
