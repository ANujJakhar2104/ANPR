...
def detect_video(video_path: str, csv_output_path: str) -> dict:
    """Returns the in-memory results dict too (not just the CSV on disk), so
    run_full_video_pipeline can build Detection rows without re-parsing what
    was just written."""
    coco_model, plate_model = _get_models()
    mot_tracker = Sort()
...
def extract_best_detections(results: dict) -> list[dict]:
    """
    Collapses the frame-by-frame results dict to one row per unique car_id -
    whichever frame had the highest OCR confidence for that car - which is
    what gets persisted as a Detection row for a video job.
    """
    best_by_car: dict = {}
    for frame_nmr, cars in results.items():
        for car_id, entry in cars.items():
            lp = entry.get('license_plate')
            if not lp:
                continue
            score = lp.get('text_score') or 0
            existing = best_by_car.get(car_id)
            if existing is None or score > (existing['license_plate'].get('text_score') or 0):
                best_by_car[car_id] = entry

    detections = []
    for car_id, entry in best_by_car.items():
        lp = entry['license_plate']
        detections.append({
            "car_id": int(car_id),
            "car_bbox": [float(v) for v in entry['car']['bbox']],
            "license_plate_bbox": [float(v) for v in lp['bbox']],
            "license_plate_bbox_score": float(lp['bbox_score']),
            "license_number": lp['text'],
            "license_number_score": float(lp['text_score']) if lp['text_score'] else None,
        })
    return detections
...
def run_full_video_pipeline(video_path: str, work_dir: str) -> dict:
    ...
    results = detect_video(video_path, csv_path)
    ...
    return {
        ...
        "detections": extract_best_detections(results),
    }